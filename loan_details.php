    ...
    <div class="row-fluid">
      <div class="span6">
        ...
        <div class="widget-title"> <span class="icon"> <i class="icon-align-justify"></i> </span>
          <h5>Welfare Loan Information</h5>
        </div>
	  ...
	  <div class="span6">
        ...
        <div class="widget-content nopadding">
          <form action="javascript:void(0);" id="form_property" class="form-horizontal">
            
            <div class="control-group">
              <label class="control-label">Loan Amount :</label>
              <div class="controls">
                <input type="text" class="span4" placeholder="Loan Amount" name="p_loan_amount" />
              </div>
			  <input type="hidden" name="pid" value="<?php if(isset($_GET['pid']) == true){ echo $pid; } ?>" />
            </div>
			...
			<div class="control-group">
              <label class="control-label">Date of Obtained :</label>
              <div class="controls">
                <input type="date" class="span4" placeholder="Date" name="p_date_obtained" />
              </div>
            </div>
			...
			<div class="control-group">
              <label class="control-label">Date of Completion :</label>
              <div class="controls">
                <input type="date" class="span4" placeholder="Date" name="p_date_completion" />
              </div>
            </div>
	  ...
	  <!--end Secondary language  -->
        ...
        <div class="widget-box">
		  ...
		  <?php
		  $sql = "SELECT * FROM property_loan_info_tbl WHERE personal_id='$pid' and status='1' ORDER BY property_loan_info_id  DESC";
		  $result = mysqli_query($conn, $sql); 
		  ?>
            ...
            <table class="table table-bordered data-table">
			  ...
			  <?php $i = 1; while ($row = mysqli_fetch_assoc($result)){ ?>
                <tr class="gradeX">
                  <td><?php echo $i; ?></td>
				  <td><?php echo $row['p_loan_amount']; ?></td>
                  <td><?php echo $row['p_date_obtained']; ?></td>
                  <td><?php echo $row['p_date_completion']; ?></td>
				  <td class="center"><button class="btn btn-success btn-mini">Update</button></td>
				  <td class="center"><button class="btn btn-warning btn-mini">Delete</button></td>
				 </tr>
                <?php $i++; } ?>
...
$("#form_submit").on('submit', (function(e) {
	...
	$("#form_property").on('submit', (function(e) {
      e.preventDefault();
      $.ajax({

        url: "func/p_loan_func.php",
        type: "POST",
        dataType: 'json',
        data: new FormData(this),
        contentType: false,
